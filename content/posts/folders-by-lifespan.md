...
정리 규칙이 지켜지려면 정리하는 순간이 아니라 만드는 순간에 판단이 끝나 있어야 한다. 만들 때 한 번 정한 걸 나중에 다시 정하지 않는 구조가 오래 간다.

## 따라 해보기

폴더 구조가 복잡해지고 있다면, 폴더 이름을 종류 대신 수명으로 바꿔 보면 된다. 이번 분기만 쓸 것, 계속 쓸 것, 날짜 기록. 세 개면 대개 충분하다.
